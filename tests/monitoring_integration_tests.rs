...
async fn check_monitoring_stack_health(
...
async fn test_metrics_exposure() {
            ...
            .await
        {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    assert!(
                        metrics_text.contains("dns_queries_total"),
                        "dns_queries_total metric should be present for {}",
                        server.name
                    );
                    assert!(
                        metrics_text.contains("dns_responses_total"),
                        "dns_responses_total metric should be present for {}",
                        server.name
                    );
                    assert!(
                        metrics_text.contains("dns_query_duration_seconds"),
                        "dns_query_duration_seconds metric should be present for {}",
                        server.name
                    );

                    // Verify instance labels are present
                    assert!(
                        metrics_text.contains(&format!("instance=\"{}\"", server.name)),
                        "Instance label should be present in metrics for {}",
                        server.name
                    );
            ...
            Err(e) => {
                println!(
                    "⚠ Failed to access metrics endpoint for {}: {}",
                    server.name, e
                );
            }
        }
    ...
    println!(
        "✓ Metrics exposure test completed: {}/{} servers accessible",
        servers_accessible, servers_tested
    );

    // If any servers are accessible, they should all have proper metrics
    assert!(
        servers_accessible > 0,
        "At least one DNS server should be accessible for metrics testing"
    );
}
...
async fn test_prometheus_scraping() {
    ...
    if let Err(e) = check_monitoring_stack_health(&config).await {
        println!(
            "Skipping Prometheus test - monitoring stack not available: {}",
            e
        );
        return;
    }
    ...
    assert!(
        targets_response.status().is_success(),
        "Prometheus targets API should be accessible"
    );

    let targets_json: Value = targets_response
        .json()
        .await
        .expect("Failed to parse targets JSON");
    ...
    assert!(
        dns_targets_found >= 1,
        "At least one DNS server target should be discovered"
    );
...
async fn test_multi_instance_monitoring() {
    ...
    println!(
        "Testing multi-instance monitoring with {} servers",
...
async fn test_grafana_dashboard_functionality() {
    ...
    match dashboards_response {
        Ok(response) => {
            if response.status().is_success() {
                let dashboards: Value = response
                    .json()
                    .await
                    .expect("Failed to parse dashboards JSON");
                ...
                if let Some(dashboards_array) = dashboards.as_array() {
                    ...
                    let dns_dashboards: Vec<_> = dashboards_array
                        .iter()
                        .filter(|d| {
                            d["title"]
                                .as_str()
...
async fn test_log_aggregation() {
    ...
    let query_params = [
        ...
        (
            "start",
            &start_time.timestamp_nanos_opt().unwrap_or(0).to_string(),
        ),
                    ...
                    if total_logs > 0 {
                        ...
                        for stream in result {
                            if let Some(values) = stream["values"].as_array() {
                                for entry in values {
                                    if let Some(log_line) = entry
                                        .as_array()
                                        .and_then(|a| a.get(1))
                                        .and_then(|v| v.as_str())
                                    {
                                        if log_line.contains("DNS")
                                            || log_line.contains("query")
                                            || log_line.contains("response")
                                        {
                                            dns_logs_found = true;
                                            break;
                                        }
...
async fn test_service_discovery() {
    ...
    let targets_response = match targets_response {
        Ok(response) => {
            if !response.status().is_success() {
                println!(
                    "Skipping service discovery test - Targets API returned status: {}",
                    response.status()
                );
                return;
            }
        ...
        Err(e) => {
            println!(
                "Skipping service discovery test - Failed to get targets: {}",
                e
            );
            return;
        }
    ...
    if let Some(active_targets) = targets_json["data"]["activeTargets"].as_array() {
        ...
        for target in active_targets {
            if let Some(job) = target["labels"]["job"].as_str() {
                if job.contains("dns") {
                    ...
                    println!(
                        "  Instance: {}",
                        target["labels"]["instance"].as_str().unwrap_or("unknown")
                    );
                    println!(
                        "  Health: {}",
                        target["health"].as_str().unwrap_or("unknown")
                    );
                    println!(
                        "  Last Scrape: {}",
                        target["lastScrape"].as_str().unwrap_or("unknown")
                    );
                }
            }
        }
        ...
        assert!(
            !dns_targets.is_empty(),
            "Should discover at least one DNS server target"
        );
        assert!(
            healthy_targets > 0,
            "At least one DNS target should be healthy"
        );

        println!(
            "✓ Service discovery working: found {} DNS targets, {} healthy",
            dns_targets.len(),
            healthy_targets
        );
