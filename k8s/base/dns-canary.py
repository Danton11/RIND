...
class DNSCanary:
    ...
    def _poll_until(self, url, expected_status, label, max_wait=5.0, interval=0.2):
        """Poll a GET endpoint until the expected status code or timeout.

        In kubernetes mode, writes go through the K8s API and the CRD watcher
        syncs them to LMDB asynchronously. With multiple pods behind a
        load-balanced service, the request may hit a pod whose watcher
        hasn't received the event yet. 5s accommodates the worst case.
        """
    ...
    def _crud_cycle_for_type(self, list_url, record_type, variant):
        ...
        # GET by id (poll for eventual consistency)
        get_response = self._poll_until(
            get_url, 200, f"Get {record_type} record"
        )
        ...
        # Verify gone (poll for eventual consistency)
        verify_response = self._poll_until(
            get_url, 404, f"{record_type} deletion verification"
        )
