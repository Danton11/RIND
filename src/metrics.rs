...
impl DnsMetrics {
    ...
    pub fn new() -> Result<Self, prometheus::Error> {
        ...
        let responses_total = CounterVec::new(
            Opts::new(
                "dns_responses_total",
                "Total number of DNS responses by code",
            ),
            &["response_code", "instance"],
        )?;

        let nxdomain_total =
            Counter::new("dns_nxdomain_total", "Total number of NXDOMAIN responses")?;

        let servfail_total =
            Counter::new("dns_servfail_total", "Total number of SERVFAIL responses")?;

        let server_uptime =
            Gauge::new("dns_server_uptime_seconds", "DNS server uptime in seconds")?;

        let active_connections =
            Gauge::new("dns_active_connections", "Number of active DNS connections")?;
        ...
        let record_operations_total = CounterVec::new(
            Opts::new(
                ...
                "Total number of record operations by type and status",
            ),
            &["operation", "status"],
        )?;
        ...
        let records_created_total =
            Counter::new("records_created_total", "Total number of records created")?;

        let records_updated_total =
            Counter::new("records_updated_total", "Total number of records updated")?;

        let records_deleted_total =
            Counter::new("records_deleted_total", "Total number of records deleted")?;

        let record_operations_failed_total = CounterVec::new(
            Opts::new(
                "record_operations_failed_total",
                "Total number of failed record operations by type and error",
            ),
        ...
        let api_requests_total = CounterVec::new(
            Opts::new(
                "api_requests_total",
                "Total number of API requests by endpoint, method, and status",
            ),
        ...
        let api_errors_total = CounterVec::new(
            Opts::new(
                "api_errors_total",
                "Total number of API errors by endpoint and error type",
            ),
            &["endpoint", "error_type"],
        )?;
